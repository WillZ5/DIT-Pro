...
DIT Pro uses semantic versioning:
...
Before publishing or producing an external installer, update all three files:
...
- `distribution`: any DMG/EXE, GitHub Release, or `latest*.json` update must confirm all three versions match.
...
- Whether the version must change.
...
- Whether the version changed. If not, why not.
- Milestone impact.
