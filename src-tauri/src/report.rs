...
<title>DIT System — Day Report {date}</title>
<style>
...
"#,
    ...
    if !report.jobs.is_empty() {
        ...
        for job in &report.jobs {
            ...
            html.push_str(&format!(
                ...
                files = format!("{}/{}", job.completed_files, job.total_files),
                size = format_bytes(job.total_bytes),
    ...
    html.push_str(&format!(
        "<div class=\"footer\">Generated by DIT System at {}</div>\n</body>\n</html>",
...
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>DIT System — Job Report: {name}</title>
<style>
  body {{ font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;
